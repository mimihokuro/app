...
function AspectRatioCalculator() {
  usePageMetadata({
    title: "アスペクト比計算ツール（画像比率・アスペクト比の計算） | EC Tool Crate",
    description:
      "画像の横幅や縦幅からアスペクト比（画面比率）を自動計算し、比率に応じたピクセルサイズを算出します。ECバナーやSNS画像、動画の縦横比の確認・計算に最適です。",
    canonicalUrl: "https://ec-tool-crate.com/aspect-ratio-calculator",
    ogTitle: "アスペクト比計算ツール（画像比率・アスペクト比の計算） | EC Tool Crate",
    ogDescription:
      "画像の横幅や縦幅からアスペクト比（画面比率）を自動計算し、比率に応じたサイズを算出します。",
    ogType: "website"
  });
  ...
  const guideData = {
    ...
    logicSteps: [
      ...
      {
        title: "横幅と比率からの高さ計算",
        formula: "高さ = 指定した横幅 × (比率の高さ ÷ 比率の幅)",
        description: "アスペクト比を維持したまま、指定した横幅に対して対応する高さをピクセル単位で算出します。",
        example: "比率 3:2 で横幅を600pxにする場合 → 600 × (2÷3) = 400px",
      },
      {
        title: "高さと比率からの横幅計算",
        formula: "横幅 = 指定した高さ × (比率の幅 ÷ 比率の高さ)",
        description: "バナー枠などの高さが決まっている場合に、アスペクト比に合わせた横幅を算出します。",
      },
    ],
    benchmarkTable: {
    ...
    faqs: [
      ...
      {
        question: "アスペクト比を維持せずに画像サイズを変更するとどうなりますか？",
        answer:
